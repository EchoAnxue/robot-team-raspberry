...
def setup():
    ...
    GPIO.setup(BtnPin_cycle, GPIO.IN,
               pull_up_down=GPIO.PUD_UP)  # Set BtnPin's mode is input,
...
if __name__ == '__main__':  # Program start from here
    ...
    user_list = [menu1.light, menu1.color, menu1.speed, menu1.mode, menu1.size, menu1.distance, menu1.behaviour,
                 menu1.logging]
    ...
    def menu1_0():
        ...
        try:
            while True:
                ...
                time.sleep(0.1)
                if GPIO.input(BtnPin_change) == False:
                    time.sleep(0.01)
                    if GPIO.input(BtnPin_change) == False:
                        print("change")
                        menu2_0(i)
                elif GPIO.input(BtnPin_cycle) == False:
                    time.sleep(0.01)
                    if GPIO.input(BtnPin_cycle) == False:
                        print("cycle")
                        i = (i + 1) % 8
                else:
                    time.sleep(0.01)
            ...
            while True:
                ...
                if GPIO.input(BtnPin_change) == False:
                    ...
                    if GPIO.input(BtnPin_cycle) == False:
                        print("cycle")
                        j = (j + 1) % length
                elif GPIO.input(BtnPin_select) == False:
                    time.sleep(0.01)
                    if GPIO.input(BtnPin_select) == False:
                        print("select")
                        user_list[i] = menu_list_2[i][j]
                        set(i, j)
                        print(user_list)
                elif GPIO.input(BtnPin_cancle) == False:
                    time.sleep(0.01)
                    if GPIO.input(BtnPin_cancle) == False:
                        print("cancle")
                        menu1_0()
                else:
                    time.sleep(0.01)
